...
class IpTaskService extends Service {
  ...
  // 定时任务获得ip地理位置信息
  async saveWebGetIpData() {
    const apps = await this.ctx.model.System.distinct('app_id', { type: 'web' })
      .read('sp')
      .exec();
    ...
    // 遍历
    apps.forEach(item => {
      this.saveWebGetIpDataByOne(item);
    });
  }
  ...
  // 单独一个应用的Ip地址更新
  async saveWebGetIpDataByOne(appId) {
    try {
  ...
  async getIpDataForIP_API(ip, _id, copyip, appId) {
    if (!ip || ip === '127.0.0.1') return;
    try {
      const url = `http://ip-api.com/json/${ip}?lang=${
        this.app.config.location.lang || 'zh-CN'
      }`;
      const result = await this.app.curl(url, {
        dataType: 'json',
      });
